...
export async function PreciseResponse(imagePath) {
  try {
      ...
      messages: [
        {
          ...
          content: [
            { type: 'text',
              text: `Here is a list of colors: ${colorsList} . Choose one color from this list of colors to represent this image, or say "LightGrey" if you are not sure. Here is a list of icons: ${iconsList}.  Choose one icon from this list of icons to represent the keywords in this image, or choose "Location" if you are not sure. Lastly, come up with exactly 10 keywords that describe this image. Respond exactly like this: "Icon: ", the icon chosen from the given list, "Color: ", the color chosen from the given list, "Keywords: ", the 10 keywords. Everything should be separated by a comma, and there should be no new lines (\n) in the response.`
            },
    ...
    const response2 = await openai.chat.completions.create(payload2);
